...
in the function's \textit{domain} only*. \sidenote{* This means that
  Figure~\ref{fig:levelsets2} is a bit of a lie: the level sets should
  be points in $\R^2$ rather than points in the $xy$-plane in
  $\R^3$. However, it is common practice to make such implicit use of
  the natural association $(x,y) \leftrightarrow (x,y,0)$ between
  $\R^2$ and the $xy$-plane}[-4cm]
...
then simplify until you can see that both sides are equal. \sidenote{*
...
polar coordinates of the point, respectively.* \sidenote{* Properly speaking,
  a coordinate is a \textit{function} For example, 
...
and $\phi$ do not play symmetric roles* \sidenote{* This is because
  ...
  to the positive half of the $xz$-\textit{plane}}[-4cm]
...
\begin{example}{}{pinchder}
  ...
  \begin{theo}{}{diff_linear}
    ...
    such that* \sidenote{* This equ<ation says that $L$ approximates
...
function
  ...
  $\partial D$. Let's do an example. \sidenote{* The notation
    ...
    points $p \in \R^2$ such that any small disk centered at $p$
...
draw(Label("$\mathbf{a}$"),P--P+(-1,1,0),MidnightBlue,Arrow3()); 
    ...
    \captionof{figure}{The vectors ${\bm \tau}, \mathbf{F}$, and
...
\begin{example}{}{triple20}
  \begin{minipage}[t]{0.65\textwidth} \parskip = 10 pt Suppose that
    ...
    dart hits the dartboard* $D=\R^2$ is given by \sidenote{* This
      ...
      mounted on---this realistic insofar as one can indeed hit
...
theorem is known as \textit{Stokes' theorem}.* \sidenote{* If you
  ...
  the divergence theorem as special cases}[-4mm]
...
\begin{example}{}{twopieces}
  ...
  cases as $t$ ranges from $0$ to $2\pi$. We get
    \[
      ...
      \int_{0}^{2\pi} \mathbf{F}(\cos t, -\sin t, 4) \cdot \langle
      -\sin t, -\cos t, 0 \rangle \, \d{t}
    \]
    which works out to
    \begin{align*}
      \int_{0}^{2\pi} \left[(4\cos t)^2 (4\sin t)(-4\sin t) + (4)(4\cos t) +
      0 \, \right] &\d t  + \int_{0}^{2\pi}\left[ \cos^2 t (-\sin t)^2 + (4)(-\cos t) +
...
some $b \in \R$. \sidenote{* The upper bound of the interval $(0,1)$
  is immaterial here, since we will be interested in the limit at 0}
...
Definition~\ref{defn:limmon} to the most general case: \sidenote{*
...
\captionof{figure}{The image under $f$ of the ball of radius $r$ is
  ...
  is. Therefore, the limit does not exist.} \label{fig:limitshrink}
\end{center}
...
\captionof{figure}{The image under $f$ of the ball of radius $r$ is
  ...
  \to 0$. Therefore, the limit exists and equals 3.  
...
\newpage 
  ...
  \begin{pf}
    ...
    \end{enumerate}
    \sidenote{* To be complete, we the case $v_1=0$ must be checked
