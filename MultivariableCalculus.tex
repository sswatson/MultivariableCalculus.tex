...
\sidenote{The norm of a vector is sometimes called the
...
\sidenote{Actually, vectors and points kind of \textit{are} the
  ...
  numbers. Their distinction is in how we use them and visualize
  them.} 
...
\sidenote{This is because $\phi$ measures the angle required to 
  rotate a vector freely so as to align with the positive $z$-axis, while $\theta$ measures
  the angle needed to rotate the vector \textit{about the $z$-axis} to
  get to the positive half of the $xz$-\textit{plane}. }
...
\epsilon$. \sidenote{\textit{Shadow} here means the set of points
  $(x,y,f(x,y))$ where $(x,y) \in B((a,b),\delta)$.}[-5mm]
...
\begin{solution}
  ...
  while holding $x$ fixed. Accordingly, we define \textbf{partial
    ...
    number, as the symbol $x$ usually does. }[1cm]
  \begin{align*}
...
differentiable $f:\R^3 \to \R$ and $\mathbf{u} \in \R^3$. 
  ...
  $\partial D$. \sidenote{Specifically, the direction in which }[-1cm]
  ...
  \begin{defn}{Integral over a 2D region}{twoDintegral}
    ...
    that $(i/n,j/n)$ is in the region $D$.   \sidenote{The $A$ in $dA$ stands for area.}[-25mm]
  \end{defn}
...
Theorem~\ref{th:iterated} more casually: \sidenote{Think of $dA$ merely as a reminder that the positive
...
We can calculate the integral by slicing up the region of
  ...
  \begin{align*}
    \text{mass} &= \int_1^2 \overbrace{\int_1^2 \int_1^2 (x^2 + y^2 + z^2) \, dx
                  \, dy \, dz}^{\text{mass of slice from $z$ to $z + dz$}} \\
                &= \int_1^2 \int_1^2 \left(y^{2} + z^{2} + \frac{7}{3}\right) \, dy
                  \, dz \\
                &= \int_1^2 \left(z^2 + \frac{14}{3}\right) \, dz \\
...
\begin{solution}
  \begin{minipage}{0.6\textwidth} \parskip = 0.2 in 
    ...
    \]
    because $\iiint_D 1 \, dV$ is equal to the mass of a solid
    ...
    equal to its volume.* \sidenote{Alternatively, note that
      calculating the Riemann sums approximating $\iiint_D 1 \, dV$
...
\begin{solution}
  ...
  \[
    ...
    \sidenote{\href{https://cocalc.com/projects/7925f475-a0bd-4621-b78a-466c24c7863c/files/change_of_variables.ipynb}{\cocalc}
    for help with this integral}
...
and area integrals over the region it encloses. \sidenote{Invented
  in 1854}[-1cm]
...
\sidenote{To interpret the result as an average,
  we should be using an area unit for which
  the total area of the earth's surface is 1}[-2mm]
...
$S$, denoted $\iint_S f \, dA$. \sidenote{Or scalar surface
...
\end{tcolorbox}
  ...
  \begin{table}[h!]
    ...
    \end{tabular}
    \caption{``Coordinate $=$ constant'' surfaces for each coordinate
...
\begin{exercise}[enhanced,breakable]{$\star$}{}
...
\begin{exercise}[enhanced,breakable]{}{}
